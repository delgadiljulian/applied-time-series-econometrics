...
detectar_raiz_proyecto <- function() {
  ...
  for (
    ...
    for (
      posible_raiz in posibles_raices
    ) {

      posible_raiz <- normalizePath(
        posible_raiz,
        winslash = "/",
        mustWork = TRUE
      )
  ...
  stop(
    paste0(
      ...
      "00_run_all.R debe estar dentro de la carpeta scripts.\n\n",
      "Directorio actual:\n",
      getwd()
    ),
    call. = FALSE
...
.PIPELINE_CTRL$directorio_pipeline_steps <- file.path(
...
.PIPELINE_CTRL$rutas_scripts <- file.path(
  .PIPELINE_CTRL$directorio_pipeline_steps,
...
# Fijar verbos tidyverse en el entorno compartido evita conflictos con
# funciones homónimas cargadas por otros paquetes durante el pipeline.

verbos_dplyr_globales <- c(
  "select",
  "filter",
  "mutate",
  "arrange",
...
for (
  verbo in verbos_dplyr_globales
) {
...
for (
  verbo in verbos_tidyr_globales
) {
