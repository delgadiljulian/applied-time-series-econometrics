...
#********************************************
#* Versión 18/07/2024    Encoding: UTF-8
#*
...
# reporte: Por default el programa no proporciona la salida de la 
#      regresión. Si se indica reporte=TRUE, aparece la salida 
...
# install.packages("urca")  
  ...
  Test.ADF.Ver.3=function(y,mod,sel.lags="AIC",alfa=0.10,reporte=FALSE) {
...
if(reporte==T){print(summary(salida))}

if(sel.lags=="SW"){print(paste("Metodo SW , lags = ",lag))}
if(sel.lags=="AIC"){print(paste("Método AIC , lags = ",lag))}
if(sel.lags=="BIC"){print(paste("Método BIC , lags = ",lag))}
  print(paste("Cantidad máxima de lags: ", maxlag))
  
  
if(reporte==F){print(paste("Estadístico Tau: ", round(salida@teststat[1],3)));
    print("Valores críticos");print(salida@cval[1,])}
}
# Ejemplos
# Test=Test.ADF.Ver.3(lc,2,"BIC")
# Test=Test.ADF.Ver.3(lc,2,"SW",reporte=T)
