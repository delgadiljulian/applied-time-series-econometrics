...
carpeta_logs <- file.path(
  "C:/Users/andre/Desktop/TP3_Series_Tiempo",
  "outputs",
  "logs"
)
...
if (length(archivos_manifiesto) == 0) {
  stop("No se encontraron manifiestos de ejecución.")
}

archivo_mas_reciente <- archivos_manifiesto[
  which.max(
    file.info(archivos_manifiesto)$mtime
  )
]
...
readr::write_csv(
  ...
  file.path(
    carpeta_logs,
    "00_resumen_advertencias_ultimo_pipeline.csv"
  )
)
