...
project_dir <- "C:/Users/julla/GitHub/applied-time-series-econometrics/trade-elasticities-var-vec/report_draft_b"

# se construye la ruta scripts_dir sin escribirla manualmente.
scripts_dir <- file.path(project_dir, "scripts")
required_scripts_dir <- file.path(scripts_dir, "required_scripts")
processed_data_dir <- file.path(project_dir, "data", "processed")
processed_final_data_dir <- file.path(processed_data_dir, "final_data_panel")
processed_auxiliary_data_dir <- file.path(processed_data_dir, "auxiliary_data")
output_dir <- file.path(project_dir, "outputs")
figures_dir <- file.path(project_dir, "figures")

final_panel_data_files <- c(
  "trade_elasticities_var_vec_panel_transformed_2004_2025.csv",
  "trade_elasticities_var_vec_panel_transformed_2004_2025.dta"
)
input_panel_data_file <- "trade_elasticities_var_vec_panel_transformed_2004_2025.csv"

processed_data_path <- function(file_name) {
  target_dir <- if (file_name %in% final_panel_data_files) {
    processed_final_data_dir
  } else {
    processed_auxiliary_data_dir
  }
...
dir.create(processed_final_data_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(processed_auxiliary_data_dir, recursive = TRUE, showWarnings = FALSE)
...
export_csv <- function(object, file, ...) {
  print_console_table(object, file)
...
#********************************************************
# se listan los paquetes teorico-practicos locales y
...
# arquitectura del docente para organizar el script maestro.
...
show_step_result("Paquetes de la cursada", course_packages_status)
show_step_result("Scripts auxiliares de clase", course_scripts_status)
show_step_result("Alineacion con el flujo de clase", course_workflow_alignment)
...
input_panel_file <- processed_data_path(input_panel_data_file)
...
show_step_result("Muestra de trabajo", model_sample_definition)
...
show_step_result("Muestras por sistema", system_sample_summary)
show_step_result("Cobertura por variable", variable_coverage)
show_step_result("Chequeo de variables requeridas", variable_check)
...
descriptive_interpretation <- data.frame(
  ...
  evidence = c(
    paste(
      "El grafico comparado en indices base 2004Q1=100 permite observar",
      "tendencias comunes de largo plazo y cambios de pendiente en las series."
    ),
    paste(
  ...
  output = c(
    "figure_01_index_comparison.png",
...
show_step_result("Estadisticas descriptivas", descriptive_stats)
show_step_result("Estacionalidad por trimestre", seasonal_diff_summary)
show_step_result("Guia de interpretacion descriptiva", descriptive_interpretation)

# abre un dispositivo grafico interactivo cuando el script se corre en RStudio
# o en una sesion R interactiva.
open_interactive_plot_device <- function(width, height, res = 180) {
  if (!interactive()) {
  ...
  if (.Platform$OS.type == "windows") {
    grDevices::windows(width = width_in, height = height_in)
  } else {
    grDevices::dev.new(width = width_in, height = height_in)
  }

  invisible(TRUE)
}

render_plot_to_file_and_screen <- function(output_file, width, height,
                                           draw_fun, res = 180) {
  draw_once <- function() {
  ...
  if (interactive()) {
    open_interactive_plot_device(width, height, res)
    draw_once()
  }
...
plot_series_panel <- function(data, variables, labels, ylab, file_name,
  ...
  render_plot_to_file_and_screen(output_file, width, height, draw_plot)
}

plot_index_comparison <- function(data, variables, labels, file_name,
                                  width = 1800, height = 1050,
                                  event_dates = NULL) {
  output_file <- file.path(figures_dir, file_name)
  series_index <- lapply(variables, function(variable_name) {
    x <- as.numeric(data[[variable_name]])
    first_valid <- which(!is.na(x))[1]
    idx <- rep(NA_real_, length(x))
    if (!is.na(first_valid)) {
      idx <- exp(x - x[first_valid]) * 100
    }
    idx
  })

  y_values <- unlist(series_index, use.names = FALSE)
  ...
  line_widths <- c(2.0, 1.7, 1.7, 1.7, 1.7, 1.7)

  draw_plot <- function() {
    par(mar = c(5.8, 4.5, 3.6, 1.2), xpd = NA)
    plot(
      data$quarter_date,
      series_index[[1]],
      type = "n",
      ...
      xlab = "",
      ylab = "Indice",
      main = "Evolucion comparada de las series"
    )
    mtext("Indices base 2004Q1 = 100", side = 3, line = 0.4,
          adj = 0, cex = 0.85)
    grid(col = "gray85")
    abline(h = 100, col = "gray45", lty = 3)
    if (!is.null(event_dates)) {
      abline(v = as.Date(event_dates), col = "gray75", lty = 3)
    }

    for (i in seq_along(series_index)) {
      lines(
        data$quarter_date,
        series_index[[i]],
        col = "black",
    ...
    legend(
      ...
      legend = labels,
      col = "black",
      lty = line_types[seq_along(labels)],
      lwd = line_widths[seq_along(labels)],
      ncol = 3,
      bty = "n",
      cex = 0.78
    )
...
show_step_result("ADF preliminar en niveles y diferencias", adf_results)
show_step_result("Orden preliminar de integracion", adf_order_summary)
...
show_step_result("ADF formal urca::ur.df", adf_urca_results)
show_step_result("Orden formal de integracion", adf_urca_order_summary)
...
show_step_result("ADF con script de clase Test.ADF_Ver.3.R", course_adf_results)
...
show_step_result("Clasificacion integrada de raices unitarias", unit_root_classification)
...
show_step_result("Engle-Granger - ecuaciones de largo plazo", engle_granger_equations)
show_step_result("Engle-Granger - coeficientes de largo plazo", engle_granger_long_run_coefficients)
show_step_result("Engle-Granger - pruebas sobre residuos", engle_granger_residual_tests)
show_step_result("Decision Engle-Granger", engle_granger_decision)
...
show_step_result("Modelos de corto plazo ECM/diferencias", short_run_model_summary)
show_step_result("Coeficientes de corto plazo ECM/diferencias", short_run_coefficients)
show_step_result("Terminos de ajuste ECM", ecm_adjustment_summary)
show_step_result("Elasticidades Engle-Granger y ECM", eg_ecm_elasticity_summary)
...
show_step_result("Wickens-Breusch - elasticidades de largo plazo", wickens_breusch_long_run_coefficients)
...
show_step_result("Seleccion de rezagos VAR", var_lag_selection)
show_step_result("Criterios de informacion VAR", var_lag_criteria)
show_step_result("Decision base de rezagos VAR", var_lag_decision)
show_step_result("Diagnostico de autocorrelacion por rezago", var_lag_diagnostic_grid)
show_step_result("Decision final de rezagos VAR", var_lag_diagnostic_decision)
...
plot_var_lag_serial_diagnostics <- function(diagnostic_grid, diagnostic_decision,
  ...
  render_plot_to_file_and_screen(output_file, 1800, 1050, draw_plot)
}
...
show_step_result("Johansen - prueba de traza", johansen_trace_results)
show_step_result("Johansen - maximo autovalor", johansen_eigen_results)
show_step_result("Johansen - decision de rango", johansen_rank_decision)
...
show_step_result("VECM - vectores de cointegracion", vecm_cointegration_vectors)
show_step_result("VECM - coeficientes de ajuste", vecm_adjustment_coefficients)
show_step_result("VECM - corto plazo", vecm_short_run_summary)
...
show_step_result("Diagnostico de autocorrelacion vcorr_res.R", course_vcorr_results)
...
show_step_result("Diagnostico White no-cross", course_white_no_cross_results)
...
show_step_result("Decision VECM/VAR", var_vec_treatment_decision)
show_step_result("Robustez Johansen", johansen_robustness_summary)
...
show_step_result("VAR en diferencias - resumen", var_diff_model_summary)
show_step_result("VAR en diferencias - coeficientes de exportaciones", var_diff_exports_equation_coefficients)
show_step_result("VAR en diferencias - diagnosticos", var_diff_diagnostics)
show_step_result("VAR en diferencias - raices de estabilidad", var_diff_stability_roots)
...
show_step_result("Normalidad y estabilidad final", normality_stability_summary)
...
show_step_result("Jerarquizacion final de modelos", model_hierarchy)
...
# se preparan IRF comparables para los sistemas reportables del informe.
# Importaciones se resume desde el VAR en niveles; exportaciones clasicas y
# ampliadas se resumen desde VAR en diferencias por su tratamiento operativo.
imports_levels_irf_fit <- tryCatch(
  vars::VAR(
    imports_var_matrix,
    p = get_preferred_var_lag("imports"),
    type = var_type
...
exports_augmented_var_diff_vars <- c(
  "d_ln_exports_real",
  "d_ln_pib_socios",
  "d_ln_itcrm",
  "d_ln_commodity_price_index"
...
exports_augmented_var_diff_matrix <- prepare_var_matrix(
  work_data,
  exports_augmented_var_diff_vars
)

exports_augmented_var_diff_fit <- tryCatch(
  vars::VAR(
    exports_augmented_var_diff_matrix,
    p = exports_var_diff_lag,
    type = exports_var_diff_type
  ),
  error = function(e) e
...
run_class_irf <- function(fit, system_name, model_key) {
  irf_object <- if (inherits(fit, "error")) {
    fit
  } else {
    tryCatch(
      vars::irf(
...
class_irf_results <- rbind(
  run_class_irf(
    imports_levels_irf_fit,
    system_name = "imports",
    model_key = "imports_levels_class_irf"
  ),
  run_class_irf(
  ...
  run_class_irf(
    exports_augmented_var_diff_fit,
    system_name = "exports_augmented",
    model_key = "exports_augmented_var_diff_class_irf"
  )
)

run_class_fevd <- function(fit, system_name, model_key) {
  fevd_object <- if (inherits(fit, "error")) {
    fit
  } else {
    tryCatch(
      vars::fevd(fit, n.ahead = class_extension_horizon),
      error = function(e) e
    )
  }
...
class_fevd_results <- rbind(
  run_class_fevd(
    imports_levels_irf_fit,
    system_name = "imports",
    model_key = "imports_levels_class_fevd"
  ),
  run_class_fevd(
    exports_var_diff_fit,
    system_name = "exports",
  ...
  run_class_fevd(
    exports_augmented_var_diff_fit,
    system_name = "exports_augmented",
    model_key = "exports_augmented_var_diff_class_fevd"
  )
)
...
class_extensions_summary <- data.frame(
  ...
  system = c(
    "imports,exports,exports_augmented",
    "imports,exports,exports_augmented",
    "exports",
    "exports"
  ),
  model_key = c(
    "multi_system_class_irf",
    "multi_system_class_fevd",
...
show_step_result("IRF de clase", class_irf_results)
show_step_result("FEVD de clase", class_fevd_results)
show_step_result("Estado SVAR de clase", class_svar_status)
show_step_result("Estado LPIRF de clase", class_lpirfs_status)
show_step_result("Resumen de extensiones de clase", class_extensions_summary)
...
output_manifest <- data.frame(
  ...
  description = c(
    ...
    "Funciones impulso-respuesta acumuladas calculadas con vars::irf para sistemas reportables",
    "Descomposicion de varianza calculada con vars::fevd para sistemas reportables",
